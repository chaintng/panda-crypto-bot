...
function getBxApiResponse() {
  if (cache.get('bx-api-response')) {
    return Promise.resolve(cache.get('bx-api-response'))
  } else {
    return request("https://bx.in.th/api/")
      .then((output) => cache.put('bx-api-response', output, 60000, getBxApiResponse))
  }
}

function getCoindeskApiResponse() {
  if (cache.get('coindesk-api-response')) {
    return Promise.resolve(cache.get('coindesk-api-response'))
  } else {
    return request("https://api.coindesk.com/v1/bpi/currentprice.json")
      .then((output) => cache.put('coindesk-api-response', output, 60000, getCoindeskApiResponse))
  }
}
