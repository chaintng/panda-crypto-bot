...
function findBxObject(bxJson, currency) {
  const currencyKey = Object.keys(bxJson).find(eachKey => bxJson[eachKey].primary_currency === 'THB' && bxJson[eachKey].secondary_currency === currency);
  return bxJson[currencyKey] || null;
}

function getSupportedCurrencies(bxJson) {
  return Object.keys(bxJson).filter(eachKey => bxJson[eachKey].primary_currency === 'THB').map(eachKey => bxJson[eachKey].secondary_currency);
}
