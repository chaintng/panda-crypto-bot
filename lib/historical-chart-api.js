...
function fetchBitcoinChart() {
  ...
  return new Pageres({ delay: 2 })
    ...
    .then(() => new Promise((resolve, reject) => {
      ...
      imgur.upload(path.join(os.tmpdir(), 'coinbasechart.png'), (err, res) => {
        console.timeEnd('upload-coinbase-chart');
        if (res.data.link) {
          debug('Crypto Chart Image', res.data.link);
          resolve(cache.putAsync('btc-chart-image', [{
            type: 'image',
...
const getChartPicture = (triggerMsg) => {
  if (triggerMsg === 'BTC') {
    if (cache.getAsync('btc-chart-image')) {
      return cache.getAsync('btc-chart-image');
    }
